...
terrain-dependent changes in real walking outcomes. We also report the loop's
physical authority boundary and its information latency honestly. All software,
assets, raw trials, and analysis code are released as a single public
repository.
\end{abstract}
...
also explicit about what is \emph{not} yet shown: the decoder-invariance of the
spike-carried information, which a companion open-loop protocol
\cite{embodied_neurocomp} demonstrates is where apparent tracking can collapse
for dynamical microcircuits. That readout-invariance analysis in the closed
loop is the clearly stated next step, not a completed claim.
...
First, the readout is fixed and hand-specified. The companion open-loop protocol
\cite{embodied_neurocomp} shows that apparent tracking of dynamical substrates
can be a readout-channel artifact: re-learning a linear readout from spike
counts changed which substrate ``carried'' the task, and for its Izhikevich
mini-culture the carried information under the re-learned readout was negative
(noise). We therefore treat decoder readout-invariance in the closed loop as
the explicit next step (our Gate~D), and we do \emph{not} claim the substrate is
the only carrier; we claim the loop is load-bearing in the sense that its
function is destroyed by the ablations above. The distinction is the paper's.
...
released as a single public repository with pinned dependencies and a bootstrap
script. No biological tissue, and no physical closed-loop platform, is required:
...
JSON files each figure is drawn from.
