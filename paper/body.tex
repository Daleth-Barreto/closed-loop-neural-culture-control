...
\cite{maass2002liquid,balci2023neurons}. A task function that demonstrably
requires the
...
is a first-order threat to such claims; our gate
(Section~\ref{sec:resG}) measures it in the closed loop. The Neural
...
\emph{Closed-loop validation.} Our falsification battery instantiates that
scrutiny as a gate structure over the loop's links
(Fig.~\ref{fig:pipeline}): each ablation removes exactly one hypothesized
mechanism, and each information-theoretic or readout-invariance test re-derives
the conclusion from the recordings instead of from the loop's appearance. The
physiological literature on population decoding
...
Every run lasts 12.5\,s (500 outer ticks) on flat ground. The task is a velocity
...
and the per-tick cadence is recorded to track the 25\,ms contract tick.
...
seeds---with large paired effect sizes (baseline $d_z=-39.2$ vs.\ zero,
...
track worse than the intact spikes. The 50\,\% lesion raises RMSE by
$+59$--$+67\,\%$ on the profile means (per-seed range $+18$--$+100\,\%$) and in
some seeds erases channel 62 entirely, which is the mechanism by which the task
signal disappears.
...
along $c_{62}\rightarrow\mathrm{cmd}$ is exactly zero for \texttt{zero},
\texttt{mask0.5}, and the dead substrate in both substrates, and nonzero for
the intact loop; the uncoupled decoder's small residual TE inherits the shared
task envelope rather than the causal link---the chain through the spikes is
present only for the substrate that carries state. Third, both quantities
reproduce---attenuated, as
...
poisson $0.307$), $p=0.0156$ on every profile and comparison, $d_z=-33.9$ vs.\
zero, $-6.7$ vs.\ random, $-3.5$ vs.\ mask0.5. The closed-loop advantage is
...
push at 0.3\,s/150\,N falls, while the mirror backward push at 0.3\,s/150\,N is
survived. These numbers bound, rather than flatter, the claim.

\textit{Latency.} The contract tick is 25\,ms of loop time, and the committed
telemetry logs the per-tick cadence of every run (the wall-clock tick interval
reflects host throughput and can exceed the 25\,ms deadline on this machine; the
round trip produce frames $\rightarrow$ read newest $\rightarrow$ decode
$\rightarrow$ stimulate remains sub-contract in loop time). The information the
command is derived
...
readout-dependence of a fixed reservoir is itself classical
\cite{maass2002liquid}; here it is a measured boundary of the closed loop
instead of a deferred caveat.
...
tooling and benchmarks made this study possible.
\fi
